...
When using a **Polars** DataFrame, the aggregation functions should be Polars expressions.

```{python}
(
    gt_sales
    .summary_rows(
        fns={"Total": lambda df: df.sum()}
    )
)
```
...
(
    ...
    .summary_rows(
        fns={
            "Total": lambda df: df.sum(),
            "Average": lambda df: df.mean(),
        }
    )
)
```
...
(
    gt_sales
    .grand_summary_rows(
        fns={"Grand Total": lambda df: df.sum()}
    )
)
```
...
(
    gt_sales
    .summary_rows(
        fns={"Subtotal": lambda df: df.sum()}
    )
    .grand_summary_rows(
        fns={
            "Grand Total": lambda df: df.sum(),
            "Overall Average": lambda df: df.mean(),
        }
    )
)
```
