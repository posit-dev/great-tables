...
The skill ships with the `great_tables` package itself, so if you already have it
installed, you already have the skill sitting on disk. Copy it into your project
with:
...
That drops it at `.claude/skills/great-tables`, ready for an agent to pick up. Point
a different agent's skill installer at the same folder, or copy it out by hand,
either works. Read `references/REFERENCE.md` first; it's the one file that tells you
where everything else in the skill lives.
