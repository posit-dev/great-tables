...
gt_ex = (GT(exibble.head(5), rowname_col="row", groupname_col="group")
...
* The argument `heading_background_color="gold"` set the heading part's background to gold.
...
This requires setting a decent number of options. The code below uses `.tab_options()` to create a blue theme, in order to help you get started with theming.
