...
Let’s use a subset of the `islands` dataset in `great_tables`:
...
The `islands` data is a simple pandas DataFrame with 2 columns.

The main entry point into the `great_tables` API is the `GT` class.
...
In the following pages we'll use **great_tables** to turn DataFrames into beautiful tables, like the ones below.
