...
Here are a number of examples that use `~~GT.vals.fmt_number()`.
...
Scientific notation can be done with `~~GT.vals.fmt_scientific()`.
...
Dates and times are handled with `~~GT.vals.fmt_date()` and `~~GT.vals.fmt_time()`.
