...
Notice two important arguments to `tab_style()`:
...
In addition to setting styles to specific values (e.g., a `"yellow"` background fill), you can also use values within table columns to specify styles.
...
In the next sections, we'll first show how this combines nicely with `cols_hide()`, and then how to use **Polars**
...
We can use a list of styles to apply multiple styles at once. For example, the code below sets fill and border styles on the same set of body cells.
