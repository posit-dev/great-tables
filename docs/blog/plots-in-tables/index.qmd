...
In the end, it's often simplest to use `svg.py`, since you can create simple charts with minimal overhead. Building elements with HTML has _even_ less overhead, but it is also slightly less user-friendly. On the other end of the spectrum, as your chart grows in complexity, using existing packages like the more exhaustive `plotnine` are good alternatives.
