...
Ultimately, the next four months were spent getting Great Tables up to snuff, and the Polars PR merged.
...
table design and tooling.

One thing that drew me to Great Tables is the sheer amount of time Rich Iannone has spent researching table display. This might not be immediately obvious, because much of his work before Great Tables was on R libraries. For example, Rich's version of Great Tables in R is called gt (2019; see his rstudio::conf() talk).
...
Making dependencies optional was a fairly quick, mundane engineering task (discussed in this [Bring Your Own DataFrame post](../bring-your-own-df/index.qmd)).
However, one important related task was **sorting out how we use Polars selectors**, to ensure they didn't break down the road.
...
well enough to justify returning via `polars.DataFrame.style`. (Though not this is still marked unstable in Polars!).
