...
![](linkedin-jerry.png){width=500px}
...
Ultimately, Great Tables used a reasonable design for table styling, carried few dependencies, and was engineered such that it could be returned from a `DataFrame.style` property.

## Making `.style` work

It seems like there were 3 pieces:
...
One thing that drew me to Great Tables is the sheer amount of time Rich Iannone has spent researching table display. This might not be immediately obvious, because much of his work before Great Tables was on R libraries.

But once you start digging, the history in R is kind of bananas. There is DT (2014), table1 (2017), flextable (2017), and reactable (2019). Rich's version of Great Tables in R is called gt (2019; see his rstudio::conf() talk). My favorite aspect of `gt` is that the community ran table contests every year. Rich's passion for tables is unleashed in the post Philosophy of Great Tables.

Fred Hutch workshop.


### Dependencies and technical work

* BYODF
* GT constructor property compat (`.tab_stub()`)
* Working out selectors in Polars issue.
* R pipe mention

## What's next?

* Someday: footnotes and summaries
* Interactive tables
* Table validation
