...
(
    GT(exibble_mini).cols_width(
        cases={"num": "30px", "char": "100px", "date": "100px", "datetime": "200px", "row": "50px"}
    )
)
```

You don't have to define widths for all columns with [cols_width()](`great_tables.GT.cols_width`), and you're free to use either `px` or `%` values when defined widths. See the [reference page](`great_tables.GT.cols_width`) for more information and relevant examples.
...
Here's an example that creates a table with a few common components and then uses [tab_options()](`great_tables.GT.tab_options`) to set up a collection of fonts for the table with the (also new) `system_fonts()` function:
...
We plan to add more `opt_*()` methods in future releases.
...
The [fmt_image()](`great_tables.GT.fmt_image`) method supports 3 kinds of files as inputs, either: (1) complete http/https or local paths to the files; (2) the file names, where a common path can be provided via the `path=` arg; or (3) a fragment of the file name, as shown in the example above.

The package has some graphics stored in the `data/metro_images` directory. They are SVGs and they look *very* nice in this table!
