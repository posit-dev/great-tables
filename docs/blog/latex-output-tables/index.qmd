...
- `\caption*{` produces our title and subtitle
- the `\multicolumn{` statements produce spanners (i.e., labels on top of multiple column labels)
- the values in the data are escaped, using `\` (e.g., `\$` represents a literal dollar sign)
...
Producing LaTeX table code is especially handy when using Quarto to generate PDF documents.

For example, the quarto document below generates a PDF from Great Tables.
...
data_dict = {
    "epic": [
        ...
        "LaTeX table code generation",
        "Rendering of stub and row labels",
        "Conversion of Markdown text to LaTeX",
        "Implementation of Units Notation in LaTeX",
        "Allow `fmt_markdown()`, `fmt_units()`, `fmt_image()`, and `fmt_nanoplot()` in LaTeX output",
        "Enable use of the `sub_missing()` and `sub_zero()` methods",
        "Document usage of options in `tab_options()` in LaTeX output context",
    ],
    "status": ["DONE", "DONE", "DONE", "TODO", "TODO", "TODO", "TODO", "TODO", "TODO"],
}
...
(
    GT(df)
    .fmt_markdown(columns="epic")
    .opt_table_font(font=google_font(name="Atkinson Hyperlegible"))
    .opt_all_caps()
    .tab_options(table_width="450px")
    .tab_style(
        style=style.fill(color="palegreen"),
        locations=loc.body(columns="status", rows=pl.col("status") == "DONE"),
    )
    .tab_style(
        style=[style.fill(color="tomato"), style.text(color="white")],
        locations=loc.body(columns="status", rows=pl.col("status") == "TODO"),
    )
