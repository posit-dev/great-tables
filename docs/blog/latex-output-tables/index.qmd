...
We've been doing quite a bit of work on getting **Great Tables** to produce LaTeX output tables and `v0.14.0` introduces the `as_latex()` method to write tables as LaTeX code. For those publishing workflows involving LaTeX documents, it's useful to have a reproducible solution for generating data tables as native LaTeX tables.
...
We wanted to get off to a good start on this and a big chunk of the work involves LaTeX output that is well-tested and consistently error-free when processed by a LaTeX engine.
