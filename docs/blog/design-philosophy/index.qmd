...
- **Table Header**: a place for a title and subtitle, where you can describe the table content
- **Column Labels**: column labels define the content of each column, spanners are headings over groups of columns
...
Notice that there is a blue row stub component that makes the row labels distinct from the rest. This is important because each person described forms a unique observation. The heading provides context on what's contained within the table. Column spanners put the columns into sensible groupings (e.g., 'Location'). The consistent use of blue lines and cell backgrounds gives the table a professional look.

If you look at the code above you'll see that every method for modifying the table starts with `tab_`. These different methods are concerned with adding a table component (e.g., `tab_header()` creates a **Table Header**) and they're designed to be easy and straightforward to use.

Table structuring is important, but not the only thing. Tables in different disciplines have certain set of display requirements specific to the values shown. Even something as simple as a number can be formatted in many different ways depending on a community's norms and expectations. This extends to a wide area considering that we also have to format dates, times, and currencies.
...
- German locale (`fmt_number()`): 134.000,00
- compact integer value (`fmt_integer()`): 134K

The problem grows worse when values need to be conveyed as images or plots. If you're a medical analyst, you'll need to effectively convey whether test results are getting better or worse over time. Reading test data as a sequence of numbers across cells can slow interpretation. But by using *nanoplots*, available as the `fmt_nanoplot()` formatting method, readers can spot trends right away. Here's an example that provides tests results over a series of days.
...
Notice that if you hover over the data points, you still get values for each of the days. Nanoplots are stripped down plots that balance the quick visual interpretation of a plot against the compactness of a table. If you peeked at the code in the above table displays you might haved noticed there are methods beginning with `fmt_` (i.e., `fmt_date()`, `fmt_integer()`, `fmt_nanoplot()`). These are the formatting methods and we designed them to be easy but to also give you great power and flexibility for your formatting tasks.
...
The publication of results is a entirely different task, and the emphasis here is on structuring, formatting, and styling. Beautiful table displays:

- make information easier to digest
- add extra context where needed
- match the style of the organization or document

We wanted to help the type of user that wanted to present data in this way. This is typically what you see in journal articles, in books, and in periodicals like [*The Economist*](https://www.economist.com/economic-and-financial-indicators/2024/03/14/economic-data-commodities-and-markets). We believe the area of static summary tables deserves it's own focus. Those tables look *great* and we offer various `opt_*()` methods in the **Great Tables** API so it's that much easier to provide a great display to your readers.
