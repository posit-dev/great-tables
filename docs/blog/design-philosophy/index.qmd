...
You're free to keep using methods to iteratively modify the table. For our part, we'll try to keep the API consistent and easy to use (but we'll provide power when you need it). Across the API surfaces of table structuring, cell formatting, styling, and output there's a lot of room for growth. We measure success by the quality of the tables the package is able to produce. We're excited about where things are going with this and we always appreciate community feedback. If ever you want to talk tables with us, you're always welcome to jump into our [Discord Server](https://discord.com/invite/Ux7nrcXHVV) and drop us a line!
