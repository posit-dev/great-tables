...
Great Tables empowers you to style and customize your table. Use format… (include code prefix). Use style…. You can add elements like nanoplots and heatmaps. 

Great Tables is flexible, providing a growing set of output formats.

You build with Great Tables iteratively, starting off with your table body from code, adding styling, formatting and other components. 
...
The problem grows worse when values need to be conveyed as images or plots. If you're a medical analyst, you'll need to know quickly whether test results are getting better or worse over time. Reading test data as a sequence of raw numbers can be slow to interpret. But with *nanoplots* we can spot trends right away. Here's an example that provides tests results over a series of days.
...
Notice that if you hover over the data points, you still get values for each of the days. Nanoplots are stripped down plots that balance the quick visual interpretation of a plot against the compactness of a table.
