...
Okay, it's not all *that bad* but the workers of data and creators of tables are today confronted with a dilemma: copy your data into a tool like Excel to format the data and otherwise make presentable by hand, or, display a sad grid of a table. This article explores the qualities and characteristics of a table (especially what makes them shine), the history of tables as a display of data, and how we might begin to solve the **great table dilemma** with **Great Tables**.
...
This set of requirements seems like the bare minimum for a table to be construed as a table. If we have one without the other, then what results is something probably not recognizable as a table.
...
With computing technologies becoming more accessible by the 1970s and 1980s, people were able to generate tables in both electronic and print form. The democratization of computational tables arguably began with VisiCalc in 1979, a massive success that initiated the computing category of spreadsheeting software.
