...
That very first table we showed here was to teach you what a table is (as if you don't know!) but I suppose it could also serve as an example of a very simple table. Let's start from that and get **Great Tables** to do its thing with its friendly and straightforward API. We'll add table components with the `tab_*()` methods, format some data with the `fmt_*()` methods, and tweak the look of the table with some `opt_*()` methods. Here goes:
