---
title: "Great Tables: Pandas now an Optional Dependency"
...
We're happy to share that Great Tables v0.5.0 makes Pandas an optional dependency.
Using Pandas DataFrames as inputs is still fully supported.
The optional dependencies simply allow users of one DataFrame library to not have to install the others.
...
* **Small**: utility libraries for things like web colors, and datetime localization.
* **Big**: a lingering dependency on numpy in a few places (like nanoplots).

For small utilities, we depend on Babel, which makes it easier to say things like, "format this number as if I'm in Germany". We also depend on webcolors, which allows us to know that "goldenrod" is the hex color "#daa520".
...
We also still really on pandas for datasets in `great_tables.data`, but will remove it soon (see [this issue](https://github.com/posit-dev/great-tables/issues/91)).

Removing dependencies like numpy and Pandas helps people who are in restricted computing environments, want a more lightweight install, or who are stuck depending on an much earlier version of a package.
...
While databackend removes dependencies, the use of singledispatch from the built-in functools module separates out the logic for handling Polars DataFrom from the logic for Pandas DataFrames.
...
For more on the benefits of singledispatch in data tooling, see the blog post [Single Dispatch for Data Science Tools](https://mchow.com/posts/2020-02-24-single-dispatch-data-science/).
...
* [Guide: selecting columns with polars expressions](/get-started/column-selection.qmd#using-polars-selectors)
