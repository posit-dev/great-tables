...
Although it really doesn't appear the have separate locations, the table header (produced by way of `tab_header()`) indeed has two of them: the title and the subtitle. These can be targeted via `loc.title()` and `loc.subtitle()`.
...
gt_tbl = (
    ...
    .tab_style(
        style=[
            style.css(rule="padding-top: 5px;"
            "padding-bottom: 5px;"
...
The combined location helper for the title and the subtitle is `loc.header()`. As mentioned before, it can be used as a shorthand for `locations=[loc.title(), loc_subtitle()]` and it's useful here where we want to change the font for the title and subtitle text.
