...
title: "Using Polars to Win at Superbowl Squares"
author: "Michael Chow"
date: 2024-02-08
freeze: true
---
...
The Superbowl is upon us, and with it the glittering squares of chance.
Maybe you've seen Superbowl Squares at your work.
Maybe you've played it with your pals.
Or maybe you have no idea what it is.

Whether you're a Square-head or not, this post will help you win with data.

## What is Superbowl Squares?

Superbowl Squares is a betting game, where you bet on the final digits of each team in a game.
...
df = (
    ...
    .join(
        pl.DataFrame({"y": list(range(10)), "z": "_._"}), how="cross"
    )
    .with_columns(
        z = pl.when((pl.col("x") == 7) & (pl.col("y") == 4))
              .then(pl.lit("4/7")).otherwise("z")
    )
...
(
    ...
    .tab_style(style.fill("lightgreen"), loc.body(columns = "4", rows=pl.col("index") == 7))
    .cols_hide("index")
    .tab_stubhead("Away")
)
```
...
and hopefully take home some combination of money and glory. (TODO: some link out to pool payout rules?).

## Why analyze squares?

Not all options in a Superbowl Square are created equal.
...
We looked back at games for the KC Chiefs (away), and games for the San Francisco 49ers (home), and calculated the proportion of the time each team ended with a specific digit.
Putting this together for the two teams, here is the chance of winning on a given square:
...
Note that the probability is conditioned on the teams playing in the superbowl. In order to estimate this,
