...
class LocFooter(Loc):
    ...
    With `loc.footer()` we can target the table's footer, which contains both the footnotes and
    source notes sections. This is useful when applying custom styling with the
    [`tab_style()`](`great_tables.GT.tab_style`) method. That method has a `locations=` argument and
    this class should be used there to perform the targeting. The 'footer' location encompasses
    content generated by both [`tab_footnote()`](`great_tables.GT.tab_footnote`) and
    [`tab_source_note()`](`great_tables.GT.tab_source_note`). To target only one of these sections,
    use [`loc.footnotes()`](`great_tables.loc.footnotes`) or
    [`loc.source_notes()`](`great_tables.loc.source_notes`) instead.
    ...
    [`tab_source_note()`](`great_tables.GT.tab_source_note`)) and style the entire footer section
    inside of [`tab_style()`](`great_tables.GT.tab_style`) with `locations=loc.footer()`.
...
class LocFootnotes(Loc):
    ...
    With `loc.footnotes()`, we can target only the footnotes in the table's footer. This is useful
    when applying custom styling with the [`tab_style()`](`great_tables.GT.tab_style`) method. That
    method has a `locations=` argument and this class should be used there to perform the targeting.
    The 'footnotes' location is generated by
    [`tab_footnote()`](`great_tables.GT.tab_footnote`). To target both the footnotes and source
    notes sections together, use [`loc.footer()`](`great_tables.loc.footer`) instead.

    Returns
    -------
    LocFootnotes
        A `LocFootnotes` object, which is used for a `locations=` argument if specifying the
        footnotes section.

    Examples
    --------
    Let's use a subset of the `gtcars` dataset in a new table. We'll add a footnote (with
    [`tab_footnote()`](`great_tables.GT.tab_footnote`)) and a source note, then style only the
    footnotes section inside [`tab_style()`](`great_tables.GT.tab_style`) with
    ...
    Styling footnotes separately from source notes lets you visually distinguish the two sections.
    Here we apply different background fills to each.
