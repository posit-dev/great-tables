...
# Generate a GT object for assertion testing
pd_data = pd.DataFrame({"x": [1, 2, 3]})
...
@pytest.fixture
def gt_tbl():
    gt_tbl = GT(pd.DataFrame({"x": [1, 2, 3]})).tab_options(
        container_width=css_length_val_large,
        container_height=css_length_val_large,
...
def test_options_all_available(gt_tbl: GT):
    assert gt_tbl._options.container_width.value == css_length_val_large
    assert gt_tbl._options.container_height.value == css_length_val_large
