...
The `islands` dataset is a simple pandas DataFrame with two columns. We've cut it down to the first ten rows and produced the `islands_mini` table. We'll feed that table to the `GT` class of `great_tables`; that serves as the main entry point into the `great_tables` API.
